...
run wget http://ftp.gnu.org/gnu/binutils/binutils-2.26.tar.gz http://ftp.gnu.org/gnu/binutils/binutils-2.26.tar.gz.sig
run gpg --verify binutils-2.26.tar.gz.sig || exit 1
run tar zxfv binutils-2.26.tar.gz
for TARGET in i386 x86_64 aarch64 alpha avr arm mips msp430 powerpc m68k sparc vax; do
  ...
  ../binutils-2.26/configure --prefix=/opt/binutils-$TARGET \
